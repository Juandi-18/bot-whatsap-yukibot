export const bodyMenu = "";
export const commands = {
  economia: `» ˚୨•(=^●ω●^=)• ⊹ \`ECONOMY\` ⊹
> ✐ Comandos de Economía para ganar dinero y divertirte con tus amigos.

ꕤ \`$prefixw\` » \`$prefixwork\` » \`$prefixtrabajar\`
> Ganar coins trabajando.
ꕤ \`$prefixbalance\` » \`$prefixbal\` » \`$prefixcoins\`
> Ver cuantos coins tienes.
ꕤ \`$prefixcoinflip\` » \`$prefixflip\` » \`$prefixcf\`
> Apostar coins en un cara o cruz.
ꕤ \`$prefixcrime\` » \`$prefixcrimen\`
> Ganar coins rapido.
ꕤ \`$prefixdaily\` » \`$prefixdiario\`
> Reclamar tu recompensa diaria.
ꕤ \`$prefixdeposit\` » \`$prefixdep\` » \`$prefixd\`
> Depositar tus coins en el banco.
ꕤ \`$prefixeconomyboard\` » \`$prefixbaltop\`
> Ver el ranking de usuarios con más coins.
ꕤ \`$prefixcasino\` » \`$prefixapostar\` » \`$prefixslot\`
> Apostar coins en el casino.
ꕤ \`$prefixeconomyinfo\` » \`$prefixeinfo\`
> Ver tu información de economía en el grupo.
ꕤ \`$prefixgivecoins\` » \`$prefixpay\`
> Dar coins a un usuario.
ꕤ \`$prefixroulette\` » \`$prefixrt\` » \`$prefixruleta\`
> Apostar coins en una ruleta.
ꕤ \`$prefixslut\` » \`$prefixprostituirse\`
> Ganar coins prostituyéndote.
ꕤ \`$prefixsteal\` » \`$prefixrob\`
> Intentar robar coins a un usuario.
ꕤ \`$prefixwithdraw\` » \`$prefixwith\` » \`$prefixretirar\`
> Retirar tus coins en el banco.
ꕤ \`$prefixminar\` » \`$prefixmine\`
> Realizar trabajos de minería y ganar coins.
ꕤ \`$prefixcofre\` » \`$prefixweekly\` » \`$prefixmonthly\`
> Reclamar cofres y recompensas periódicas.
ꕤ \`$prefixaventura\` » \`$prefixcurar\` » \`$prefixcazar\`
> Salir de aventura, curarse o cazar animales.
ꕤ \`$prefixfish\` » \`$prefixpescar\`
> Ganar coins pescando.
ꕤ \`$prefixmazmorra\` » \`$prefixdungeon\`
> Explorar mazmorras para ganar coins.
ꕤ \`$prefixmath\` » \`$prefixmates\`
> Iniciar un juego de matemáticas.
ꕤ \`$prefixppt\`
> Jugar piedra, papel o tijera con el bot.`,

gacha: `\n» ˚୨•(=^●ω●^=)• ⊹ \`GACHA\` ⊹
> ✐ Comandos de Gacha para reclamar e intercambiar personajes.

ꕤ \`$prefixbuycharacter\` » \`$prefixbuyc\`
> Comprar un personaje en venta.
ꕤ \`$prefixcharimage\` » \`$prefixcimage\`
> Ver una imagen aleatoria de un personaje.
ꕤ \`$prefixcharinfo\` » \`$prefixwinfo\`
> Ver información de un personaje.
ꕤ \`$prefixclaim\` » \`$prefixc\` » \`$prefixreclamar\`
> Reclamar un personaje.
ꕤ \`$prefixdelclaimmsg\`
> Restablecer el mensaje al reclamar.
ꕤ \`$prefixdeletewaifu\` » \`$prefixdelchar\`
> Eliminar un personaje reclamado.
ꕤ \`$prefixfavoritetop\` » \`$prefixfavtop\`
> Ver el top de personajes favoritos.
ꕤ \`$prefixgachainfo\` » \`$prefixginfo\`
> Ver tu información de gacha.
ꕤ \`$prefixgivechar\` » \`$prefixregalar\`
> Regalar un personaje a otro usuario.
ꕤ \`$prefixharem\` » \`$prefixwaifus\` » \`$prefixclaims\`
> Ver tus personajes reclamados.
ꕤ \`$prefixharemshop\` » \`$prefixwshop\`
> Ver los personajes en venta.
ꕤ \`$prefixremovesale\` » \`$prefixremoverventa\`
> Eliminar un personaje en venta.
ꕤ \`$prefixrollwaifu\` » \`$prefixrw\` » \`$prefixroll\`
> Waifu o husbando aleatorio.
ꕤ \`$prefixsell\` » \`$prefixvender\`
> Poner un personaje a la venta.
ꕤ \`$prefixserieinfo\` » \`$prefixanimelist\`
> Información y lista de series.
ꕤ \`$prefixsetclaimmsg\` » \`$prefixsetclaim\`
> Modificar el mensaje al reclamar.
ꕤ \`$prefixtrade\` » \`$prefixintercambiar\`
> Intercambiar un personaje con otro usuario.
ꕤ \`$prefixvote\` » \`$prefixvotar\`
> Votar por un personaje para subir su valor.
ꕤ \`$prefixwaifustop\` » \`$prefixwtop\`
> Ver el top de personajes con mayor valor.`,
  // --- ./core/commands.js (PARTE 2) ---

downloads: `\n» ˚୨•(=^●ω●^=)• ⊹ \`DOWNLOAD\` ⊹
> ✐ Comandos de Descargas para descargar archivos de varias fuentes.

ꕤ \`$prefixfacebook\` » \`$prefixfb\`
> Descargar un video de Facebook.
ꕤ \`$prefixmediafire\` » \`$prefixmf\`
> Descargar un archivo de MediaFire.
ꕤ \`$prefixplay\` » \`$prefixmp3\` » \`$prefixytaudio\`
> Descargar una canción de YouTube.
ꕤ \`$prefixpinterest\` » \`$prefixpin\`
> Buscar y descargar imagenes de Pinterest.
ꕤ \`$prefixplay2\` » \`$prefixmp4\` » \`$prefixytvideo\`
> Descargar un vídeo de YouTube.
ꕤ \`$prefixreel\` » \`$prefixig\` » \`$prefixinstagram\`
> Descargar un reel de Instagram.
ꕤ \`$prefixtiktok\` » \`$prefixtt\`
> Descargar un video de TikTok.
ꕤ \`$prefixtwitter\` » \`$prefixx\`
> Descargar un video/imagen de Twitter/X.
ꕤ \`$prefixytsearch\` » \`$prefixsearch\`
> Buscar videos de YouTube.
ꕤ \`$prefixwagrupos\` » \`$prefixgruposwa\`
> Buscar grupos de WhatsApp.
ꕤ \`$prefiximagen\` » \`$prefiximg\`
> Buscar y descargar imagenes de Google.
ꕤ \`$prefixaptoide\` » \`$prefixapk\` » \`$prefixapkdl\`
> Buscar y descargar aplicaciones de Aptoide.`,

profile: `\n» ˚୨•(=^●ω●^=)• ⊹ \`PROFILES\` ⊹
> ✐ Comandos de Perfil para ver y configurar tu perfil.

ꕤ \`$prefixprofile\` » \`$prefixperfil\`
> Ver tu perfil o el de un usuario.
ꕤ \`$prefixleaderboard\` » \`$prefixlb\`
> Top de usuarios con más experiencia.
ꕤ \`$prefixlevel\` » \`$prefixlvl\`
> Ver tu nivel y experiencia actual.
ꕤ \`$prefixsetgenre\` » \`$prefixdelgenre\`
> Establecer o eliminar tu genero.
ꕤ \`$prefixsetbirth\` » \`$prefixdelbirth\`
> Gestionar tu fecha de cumpleaños.
ꕤ \`$prefixsetdescription\` » \`$prefixsetdesc\`
> Establecer tu descripcion.
ꕤ \`$prefixdeldescription\` » \`$prefixdeldesc\`
> Eliminar tu descripción de perfil.
ꕤ \`$prefixmarry\` » \`$prefixcasarse\`
> Casarte con alguien.
ꕤ \`$prefixdivorce\`
> Divorciarte de tu pareja.
ꕤ \`$prefixsetfavourite\` » \`$prefixsetfav\`
> Establecer tu claim favorito.
ꕤ \`$prefixdeletefav\` » \`$prefixdelfav\`
> Borrar tu claim favorito.
ꕤ \`$prefixsetpasatiempo\` » \`$prefixsethobby\`
> Establecer tu pasatiempo.
ꕤ \`$prefixdelpasatiempo\` » \`$prefixremovehobby\`
> Elimina tu pasatiempo del perfil.`,

sockets: `\n» ˚୨•(=^●ω●^=)• ⊹ \`SOCKETS\` ⊹
> ✐ Comandos para registrar tu propio bot.

ꕤ \`$prefixbotinfo\` » \`$prefixinfobot\`
> Obtener informacion del bot.
ꕤ \`$prefixjoin\` » \`$prefixleave\`
> Unir o sacar al bot de un grupo.
ꕤ \`$prefixlogout\` » \`$prefixreload\`
> Cerrar sesión o recargar la sesión del bot.
ꕤ \`$prefixself\` + <on|off>
> Haz privado o público tu bot.
ꕤ \`$prefixqr\` » \`$prefixcode\`
> Crear un Sub-Bot con un codigo Code.
ꕤ \`$prefixsetname\` » \`$prefixsetbotname\`
> Cambiar el nombre del bot.
ꕤ \`$prefixsetbanner\` » \`$prefixsetbotbanner\`
> Cambiar el banner del menu.
ꕤ \`$prefixseticon\` » \`$prefixsetboticon\`
> Cambiar el icon del bot.
ꕤ \`$prefixsetprefix\` » \`$prefixsetbotprefix\`
> Cambiar el prefijo del bot.
ꕤ \`$prefixsetcurrency\` » \`$prefixsetbotcurrency\`
> Cambiar la moneda del bot.
ꕤ \`$prefixsetowner\` » \`$prefixsetbotowner\`
> Cambiar el dueño del bot.
ꕤ \`$prefixonlyowner\` » \`$prefixsmodoprivado\`
> Haz privado o público tu bot en un grupo.
ꕤ \`$prefixsetchannel\` » \`$prefixsetbotchannel\`
> Cambiar el canal del bot.
ꕤ \`$prefixsetlink\` » \`$prefixsetbotlink\`
> Cambiar el enlace del bot.
ꕤ \`$prefixsetpfp\` » \`$prefixsetimage\`
> Cambiar la imagen de perfil.
ꕤ \`$prefixsetstatus\` » \`$prefixsetusername\`
> Cambiar estado o nombre de usuario.`,
  // --- ./core/commands.js (PARTE 3) ---

stickers: `\n» ˚୨•(=^●ω●^=)• ⊹ \`STICKERS\` ⊹
> ✐ Comandos de *Stickers* para crear y gestionar stickers.

ꕤ \`$prefixstickerpack\` » \`$prefixspack\` » \`$prefixstickers\`
> Busca y descarga packs de Stickers.
ꕤ \`$prefixdelpack\`
> Elimina un paquete de stickers.
ꕤ \`$prefixdelstickermeta\` » \`$prefixdelmeta\`
> Restablecer el pack y autor por defecto para tus stickers.
ꕤ \`$prefixgetpack\` » \`$prefixpack\`
> Descarga un paquete de stickers.
ꕤ \`$prefixnewpack\` » \`$prefixnewstickerpack\`
> Crea un nuevo paquete de stickers.
ꕤ \`$prefixsetpackprivate\` » \`$prefixsetpackpriv\`
> Establecer un paquete de stickers como privado.
ꕤ \`$prefixsetpackpublic\` » \`$prefixsetpackpub\`
> Establecer un paquete de stickers como público.
ꕤ \`$prefixsetstickermeta\` » \`$prefixsetmeta\`
> Establecer el pack y autor por defecto para tus stickers.
ꕤ \`$prefixsticker\` » \`$prefixs\`
> Convertir una imagen/video a sticker.
ꕤ \`$prefixsetstickerpackdesc\` » \`$prefixsetpackdesc\`
> Establece la descripción de un paquete de stickers.
ꕤ \`$prefixsetstickerpackname\` » \`$prefixsetpackname\`
> Cambia el nombre de un paquete de stickers.
ꕤ \`$prefixstickeradd\` » \`$prefixaddsticker\`
> Agrega un sticker a un paquete de stickers.
ꕤ \`$prefixstickerdel\` » \`$prefixdelsticker\`
> Elimina un sticker de un paquete de stickers.
ꕤ \`$prefixstickerpacks\` » \`$prefixpacklist\`
> Lista de tus paquetes de stickers.
ꕤ \`$prefixbrat\` » \`$prefixbratv\` » \`$prefixqc\` » \`$prefixemojimix\`
> Crear stickers con texto.`,

utils: `\n» ˚୨•(=^●ω●^=)• ⊹ \`UTILITIES\` ⊹
> ✐ Comandos de Útilidades generales.

ꕤ \`$prefixmenu\` » \`$prefixhelp\` » \`$prefixayuda\`
> Ver el menú de comandos por categoría.
ꕤ \`$prefixbots\` » \`$prefixsockets\`
> Ver el número de bots activos en el sistema.
ꕤ \`$prefixdel\`+ <cantidad>
> Borra una cantidad específica de mensajes.
ꕤ \`$prefixstatus\` » \`$prefixestado\`
> Ver el estado actual y salud del bot.
ꕤ \`$prefixping\` » \`$prefixp\` » \`$prefixspeed\`
> Medir el tiempo de respuesta del bot.
ꕤ \`$prefixreport\` » \`$prefixreporte\`
> Enviar un mensaje de reporte a los moderadores.
ꕤ \`$prefixsug\` » \`$prefixsuggest\`
> Enviar una sugerencia a los moderadores.
ꕤ \`$prefixinvitar\` » \`$prefixinvite\`
> Invitar al bot a un grupo mediante un enlace.
ꕤ \`$prefixia\` » \`$prefixchatgpt\`
> Realizar peticiones a la inteligencia artificial.
ꕤ \`$prefixgetpic\` » \`$prefixpfp\`
> Ver la foto de perfil de un usuario.
ꕤ \`$prefixtoimage\` » \`$prefixtoimg\`
> Convertir sticker o imagen de una vista a imagen.
ꕤ \`$prefixtourl\`
> Convierte una imagen o video en un enlace.
ꕤ \`$prefixsay\` » \`$prefixdecir\`
> Hacer que el bot repita un mensaje de texto.
ꕤ \`$prefixtrad\` » \`$prefixtraducir\` » \`$prefixtranslate\`
> Traducir texto al idioma especificado.
ꕤ \`$prefixget\` » \`$prefixfetch\`
> Realizar solicitudes GET a páginas web.
ꕤ \`$prefixhd\` » \`$prefixenhance\` » \`$prefixremini\`
> Mejorar la calidad de una imagen.
ꕤ \`$prefixgitclone\` » \`$prefixgit\`
> Buscar y descargar un repositorio de GitHub.
ꕤ \`inspect\` » \`inspeccionar\`
> Ver información de grupos o canales de WhatsApp.
ꕤ \`$prefixread\` » \`$prefixreadviewonce\`
> Convertir imagen/video de una vista a contenido normal.`,

grupo: `\n» ˚୨•(=^●ω●^=)• ⊹ \`GROUPS\` ⊹
> ✐ Comandos para administradores de grupos.

ꕤ \`$prefixalerts\` » \`$prefixalertas\`
> Activar/desactivar las alertas de grupo.
ꕤ \`$prefixantilinks\` » \`$prefixantienlaces\`
> Activar/desactivar el antienlaces.
ꕤ \`$prefixbot\`
> Activar/desactivar al bot.
ꕤ \`$prefixclose\` » \`$prefixcerrar\`
> Cerrar el grupo por un tiempo determinado.
ꕤ \`$prefixgp\` » \`$prefixgroupinfo\`
> Información detallada del grupo.
ꕤ \`$prefixdelwarn\`
> Eliminar una advertencia de un miembro.
ꕤ \`$prefixdemote\`
> Descender a un usuario de administrador.
ꕤ \`$prefixeconomy\` » \`$prefixeconomia\`
> Activar/desactivar los comandos de economía.
ꕤ \`$prefixgacha\` » \`$prefixrpg\`
> Activar/desactivar los comandos de GACHA.
ꕤ \`$prefixgoodbye\` » \`$prefixdespedida\`
> Activar/desactivar la despedida.
ꕤ \`$prefixsetgpbaner\`
> Cambiar la imagen o banner del grupo.
ꕤ \`$prefixsetgpname\`
> Cambiar el nombre del grupo.
ꕤ \`$prefixsetgpdesc\`
> Cambiar la descripción del grupo.
ꕤ \`$prefixkick\`
> Expulsar a un usuario del grupo.
ꕤ \`$prefixnsfw\`
> Activar/desactivar los comandos NSFW.
ꕤ \`$prefixonlyadmin\` » \`$prefixadminonly\`
> Solo admins pueden usar comandos.
ꕤ \`$prefixopen\` » \`$prefixabrir\`
> Abrir el grupo por un tiempo determinado.
ꕤ \`$prefixpromote\`
> Ascender a un usuario a administrador.
ꕤ \`$prefixsetgoodbye\`
> Establecer mensaje de despedida personalizado.
ꕤ \`$prefixsetprimary\`
> Establece un bot como primario del grupo.
ꕤ \`$prefixsetwarnlimit\`
> Establecer el límite de advertencias.
ꕤ \`$prefixsetwelcome\`
> Establecer mensaje de bienvenida personalizado.
ꕤ \`$prefixtag\` » \`$prefixhidetag\` » \`$prefixtagall\`
> Mencionar a todos los miembros del grupo.
ꕤ \`$prefixmsgcount\` » \`$prefixcount\` » \`$prefixmessages\`
> Conteo de mensajes y comandos de un usuario.
ꕤ \`$prefixtopcount\` » \`$prefixtopmessages\` » \`$prefixtopmsgcount\`
> Top de usuarios con más mensajes.
ꕤ \`$prefixtopinactive\` » \`$prefixtopinactivos\`
> Top de usuarios más inactivos del grupo.
ꕤ \`$prefixwarn\`
> Darle una advertencia a un miembro.
ꕤ \`$prefixwarns\`
> Ver todas las advertencias de un miembro.
ꕤ \`$prefixwelcome\` » \`$prefixbienvenida\`
> Activar/desactivar la bienvenida.
ꕤ \`$prefixlink\` » \`$prefixrevoke\`
> Obtener o restablecer el enlace del grupo.`,

nsfw: `\n» ˚୨•(=^●ω●^=)• ⊹ \`NSFW\` ⊹
> ✐ Comandos NSFW (Contenido para adultos).

ꕤ \`$prefixxnxx\`
> Buscar y descarga videos de XNXX.
ꕤ \`$prefixxvideos\`
> Buscar y descarga videos de XVideos.
ꕤ \`$prefixdanbooru\` » \`$prefixdbooru\`
> Buscar imágenes en Danbooru.
ꕤ \`$prefixgelbooru\` » \`$prefixgbooru\`
> Buscar imágenes en Gelbooru.
ꕤ \`$prefixrule34\` » \`$prefixr34\`
> Buscar imagenes en Rule34.
ꕤ \`$prefixanal\` » \`$prefixviolar\`
> Hacer un anal.
ꕤ \`$prefixblowjob\` » \`$prefixmamada\` » \`$prefixbj\`
> Dar una mamada.
ꕤ \`$prefixboobjob\`
> Hacer una rusa.
ꕤ \`$prefixbondage\`
> Atar sin escapatoria.
ꕤ \`$prefixbukkake\`
> Hacer un bukkake.
ꕤ \`$prefixcum\`
> Venirse en alguien.
ꕤ \`$prefixcummouth\`
> Acabar en la boca de alguien.
ꕤ \`$prefixcumshot\`
> Disparar semen.
ꕤ \`$prefixcreampie\`
> Dejar un creampie.
ꕤ \`$prefixdeepthroat\`
> Hacer una garganta profunda.
ꕤ \`$prefixfacesitting\`
> Sentarse en la cara de alguien.
ꕤ \`$prefixfap\` » \`$prefixpaja\`
> Hacerse una paja.
ꕤ \`$prefixfingering\`
> Meter los dedos.
ꕤ \`$prefixfootjob\`
> Hacer una paja con los pies.
ꕤ \`$prefixfuck\` » \`$prefixcoger\`
> Follarte a alguien.
ꕤ \`$prefixfutanari\` » \`$prefixfuta\`
> Lo mejor de los dos mundos.
ꕤ \`$prefixgrabboobs\`
> Agarrar tetas.
ꕤ \`$prefixgrope\`
> Manosear a alguien.
ꕤ \`$prefixhandjob\`
> Hacer una paja.
ꕤ \`$prefixlickass\`
> Lamer un culo.
ꕤ \`$prefixlickdick\`
> Lamer un pene.
ꕤ \`$prefixlickpussy\`
> Lamer un coño.
ꕤ \`$prefixorgy\` » \`$prefixorgia\`
> Organizar una orgía.
ꕤ \`$prefixpegging\`
> Dar por detrás.
ꕤ \`$prefixsixnine\` » \`$prefix69\`
> Haz un 69 con alguien.
ꕤ \`$prefixspank\` » \`$prefixnalgada\`
> Dar una nalgada.
ꕤ \`$prefixsquirt\` » \`$prefixsquirting\`
> Tener un squirt.
ꕤ \`$prefixsuckboobs\`
> Chupar tetas.
ꕤ \`$prefixthighjob\`
> Hacer una entre piernas.
ꕤ \`$prefixundress\` » \`$prefixencuerar\`
> Desnudar a alguien.
ꕤ \`$prefixyaoi\`
> Momento muy intenso.
ꕤ \`$prefixyuri\` » \`$prefixtijeras\`
> Hacer tijeras.`,

anime: `\n» ˚୨•(=^●ω●^=)• ⊹ \`ANIME\` ⊹
> ✐ Comandos de reacciones de Anime.

ꕤ \`$prefixwaifu\` » \`$prefixneko\`
> Buscar una waifu aleatoria.
ꕤ \`$prefixppcouple\` » \`$prefixppcp\`
> Generar imágenes para amistades o parejas.
ꕤ \`$prefixpeek\` » \`$prefixmirar\`
> Mirar a alguien.
ꕤ \`$prefixcomfort\` » \`$prefixconsolar\`
> Consolar a alguien.
ꕤ \`$prefixthinkhard\` » \`$prefixpensar\`
> Pensar intensamente.
ꕤ \`$prefixcurious\` » \`$prefixcurioso\`
> Mostrar curiosidad.
ꕤ \`$prefixsniff\` » \`$prefixoler\`
> Oler a alguien.
ꕤ \`$prefixstare\` » \`$prefixmirar\`
> Mirar fijamente.
ꕤ \`$prefixtrip\` » \`$prefixtropezar\`
> Tropezar con alguien.
ꕤ \`$prefixblowkiss\` » \`$prefixbesito\`
> Mandar un besito.
ꕤ \`$prefixsnuggle\` » \`$prefixacurrucar\`
> Acurrucarse con alguien.
ꕤ \`$prefixangry\` » \`$prefixenojado\`
> Estar enojado.
ꕤ \`$prefixbleh\` » \`$prefixmeh\`
> Sacar la lengua.
ꕤ \`$prefixbored\` » \`$prefixaburrido\`
> Estar aburrido.
ꕤ \`$prefixclap\` » \`$prefixaplaudir\`
> Aplaudir.
ꕤ \`$prefixcoffee\` » \`$prefixcafe\`
> Tomar café.
ꕤ \`$prefixcold\` » \`$prefixfrio\`
> Tener frío.
ꕤ \`$prefixsing\` » \`$prefixcantar\`
> Cantar.
ꕤ \`$prefixtickle\` » \`$prefixcosquillas\`
> Hacer cosquillas.
ꕤ \`$prefixscream\` » \`$prefixgritar\`
> Gritar.
ꕤ \`$prefixpush\` » \`$prefixempujar\`
> Empujar a alguien.
ꕤ \`$prefixnope\` » \`$prefixno\`
> Negarse a hacer algo.
ꕤ \`$prefixjump\` » \`$prefixsaltar\`
> Saltar.
ꕤ \`$prefixheat\` » \`$prefixcalor\`
> Tener calor.
ꕤ \`$prefixgaming\` » \`$prefixjugar\`
> Jugar videojuegos.
ꕤ \`$prefixdraw\` » \`$prefixdibujar\`
> Dibujar.
ꕤ \`$prefixcall\` » \`$prefixllamar\`
> Llamar a alguien.
ꕤ \`$prefixdramatic\` » \`$prefixdrama\`
> Drama.
ꕤ \`$prefixdrunk\` » \`$prefixborracho\`
> Estar borracho.
ꕤ \`$prefiximpregnate\` » \`$prefibarazar\`
> Embarazar a alguien.
ꕤ \`$prefixkisscheek\` » \`$prefixbeso\`
> Beso en la mejilla.
ꕤ \`$prefixlaugh\` » \`$prefixreir\`
> Reírse de alguien.
ꕤ \`$prefixlove\` » \`$prefixamor\`
> Sentirse enamorado.
ꕤ \`$prefixpout\` » \`$prefixmueca\`
> Hacer pucheros.
ꕤ \`$prefixpunch\` » \`$prefixgolpear\`
> Dar un puñetazo.
ꕤ \`$prefixrun\` » \`$prefixcorrer\`
> Correr.
ꕤ \`$prefixsad\` » \`$prefixtriste\`
> Expresar tristeza.
ꕤ \`$prefixscared\` » \`$prefixasustado\`
> Estar asustado.
ꕤ \`$prefixseduce\` » \`$prefixseducir\`
> Seducir a alguien.
ꕤ \`$prefixshy\` » \`$prefitimido\`
> Sentir timidez.
ꕤ \`$prefixsleep\` » \`$prefixdormir\`
> Tumbarse a dormir.
ꕤ \`$prefixsmoke\` » \`$prefixfumar\`
> Fumar.
ꕤ \`$prefixspit\` » \`$prefixescupir\`
> Escupir.
ꕤ \`$prefixstep\` » \`$prefixpisar\`
> Pisar a alguien.
ꕤ \`$prefixthink\` » \`$prefixpensar\`
> Pensar en algo.
ꕤ \`$prefixwalk\` » \`$prefixcaminar\`
> Caminar.
ꕤ \`$prefixhug\` » \`$prefixabrazar\`
> Dar un abrazo.
ꕤ \`$prefixkill\` » \`$prefixmatar\`
> Toma tu arma y mata a alguien.
ꕤ \`$prefixeat\` » \`$prefixnom\` » \`$prefixcomer\`
> Comer algo delicioso.
ꕤ \`$prefixkiss\` » \`$prefixmuak\`
> Dar un beso.
ꕤ \`$prefixwink\`
> Guiñar un ojo.
ꕤ \`$prefixpat\`
> Acariciar a alguien.
ꕤ \`$prefixhappy\` » \`$prefixfeliz\`
> Saltar de felicidad.
ꕤ \`$prefixbully\`
> Molestar a alguien.
ꕤ \`$prefixbite\` » \`$prefixmorder\`
> Morder a alguien.
ꕤ \`$prefixblush\`
> Sonrojarse.
ꕤ \`$prefixwave\`
> Saludar con la mano.
ꕤ \`$prefixbath\`
> Bañarse.
ꕤ \`$prefixsmug\`
> Actuar presumido.
ꕤ \`$prefixsmile\`
> Sonreír.
ꕤ \`$prefixhighfive\`
> Dar un "choca esos cinco".
ꕤ \`$prefixhandhold\`
> Tomar de la mano.
ꕤ \`$prefixcringe\`
> Hacer una mueca incómoda.
ꕤ \`$prefixbonk\`
> Dar un golpe divertido.
ꕤ \`$prefixcry\`
> Llorar.
ꕤ \`$prefixlick\`
> Lamer a alguien.
ꕤ \`$prefixslap\`
> Dar una bofetada.
ꕤ \`$prefixdance\`
> Bailar.
ꕤ \`$prefixcuddle\`
> Acurrucarse.`,
};
