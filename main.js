...
export default async (client, m) => {
  ...
  // --- NUEVO: SISTEMA DE MEMORIA PARA EL COMANDO CLEAN ---
  if (!client.messages) client.messages = {};
  if (!client.messages[m.chat]) client.messages[m.chat] = { array: [] };
  
  // Guardamos el mensaje actual en el historial (sea comando o no)
  client.messages[m.chat].array.push(m);
  
  // Mantenemos la memoria limpia (máximo 100 mensajes por chat)
  ...
  // ... (aquí sigue tu código de plugins.all y stats de mensajes) ...
  ...
  // ... (aquí sigue tu código de prefixes y regex) ...
  ...
  // --- IMPORTANTE: Aquí estaba el error. Si no hay match, el bot se detenía. ---
  // El código de arriba ya guardó el mensaje, así que ahora sí podemos salir.
  if (!match) return;

  // ... (El resto de tu main.js queda exactamente igual) ...
  ...
  // Log de consola y ejecución de comandos (mantenlo igual a tu original)
  // ...
  ...
  // (Final de tu main.js con el bloque try/catch para ejecutar cmdData.run)
  ...
  // ... (resto de funciones como getAllSessionBots)
